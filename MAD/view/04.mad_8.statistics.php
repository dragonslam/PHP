				<div class="h2_wrap">
					<h2><?php PS_Image("txt_h2_sharelist.png", "공유리스트")?> </h2>
				</div>
				<nav class="tab_1">
					<ul>
						<li class="on"><a <?php PS_PageLink("statistics8")?>>최근 한달 공유 리스트</a></li>
						<li><a <?php PS_PageLink("statistics8a")?>>지난달 공유 리스트</a></li>
					</ul>
				</nav>
				<section class="cs1">
					<div class="h3_wrap">
						...
						<div class="fr">
							<nav class="tab_2">
								<ul>
									...
									<li><a href="#sort2">유입순</a></li>
									<li><a href="#sort3">회원가입순</a></li>
									<li><a href="#sort4">구매순</a></li>
...
$(document).ready(function(){
	...
	function getData(rowcnt, pagecnt, sort_type) {		
		 p_rowcnt = rowcnt ? rowcnt : p_rowcnt, 
		 p_pagecnt = pagecnt ? pagecnt : 0,
		 p_sort_type = sort_type ? sort_type : 0;

		oStatictis.getCurrentShare(p_rowcnt, p_pagecnt, p_sort_type, function(r) {		
			if (r.err == 0 && r.cnt > 0) {
				p_total_cnt = r.cnt;		
				p_data = r.ret;
	...
	function exportExcel(oAnchor) {
		...
		oStatictis.getLastMonthShare(row_cnt, 0, p_sort_type, function(r) {		
			if (r.err == 0 && r.cnt > 0) {
	...
	function drawList(obj, data, cnt, callback) {
		...
		if (typeof data == 'object' && data != null) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				row.append("	<th scope='row' colspan='2'>공유자</th>");
				row.append("	<th scope='row' colspan='2'>회원 ID</th>");
				row.append("	<th scope='row'>콘텐츠 제목</th>");
				row.append("</tr>");
				row.append("<tr>");
				row.append("	<td rowspan='3'>"+ (cnt + i + 1) +"</td>");
				row.append("	<td colspan='2'>"+ item.user_id +"</td>");
				row.append("	<td colspan='2'>"+ item.s_user_id +"</td>");
				row.append("	<td>"+ item.title +"</td>");
				row.append("</tr>");
				row.append("<tr>");
				row.append("	<th scope='row'>유입자 수</th>");
				row.append("	<th scope='row'>클릭수</th>");
				row.append("	<th scope='row'>회원가입 수</th>");
				row.append("	<th scope='row'>구매수</th>");
				row.append("	<th scope='row'>공유일</th>");
				row.append("</tr>");
				row.append("<tr>");
				row.append("	<td class='fc_1'>"+ String(item.view_cnt).money() +"명</td>");
				row.append("	<td class='fc_2'>"+ String(item.click_cnt).money() +"명</td>");
				row.append("	<td class='fc_3'>"+ String(item.reg_cnt).money() +"명</td>");
				row.append("	<td class='fc_4'>"+ String(item.buy_cnt).money() +"명</td>");
				row.append("	<td>"+ String(item.reg_date).toDate().toDateTimeString(11) +"</td>");
				row.append("</tr>");
				row.append("<tr>");
				row.append("	<td colspan='6' style='border-left:0px;border-right:0px;height:5px;'></td>");
				row.append("</tr>");
			}
			obj.append(row.toString(''));
		}
		else {
			obj.append("<tr><td colspan='6'>해당되는 내용이 없습니다.</td></tr>");
		}
	}
