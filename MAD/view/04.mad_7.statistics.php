				...
				<nav class="tab_1">
					<!-- ul>
						<li class="on"><a <?php PS_PageLink("statistics7")?>>최근 한달 공유된 콘텐츠</a></li>
						<!-- li><a <?php PS_PageLink("statistics7a")?>>지난달 공유된 콘텐츠</a></li -->
					</ul -->
				</nav>
				<section class="cs1">
					<div class="h3_wrap">
	...
	function drawList(obj, data, cnt, callback) {
		...
		if (typeof data == 'object' && data != null) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (String(item.title).isEmpty()) {
					row.append("	<td class='ta_l'>"+ item.title +"</td>");
				} else {
					row.append("	<td class='ta_l'><a href='"+ item.url +"' target='_blank'>"+ item.title +"</a></td>");
				}
